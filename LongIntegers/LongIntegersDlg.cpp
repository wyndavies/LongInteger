...
void CLongIntegersDlg::OnClickedIdarrow()
{
	...
	memset(bytearray, 55, sizeof(byte) * 1000);
	liTestVal1.assignByteArray(bytearray, 1000);
	delete bytearray;
	liTestVal2 = liTestVal1;
	liTestVal1 *= liTestVal1; // 2000 digits
	liTestVal2 *= liTestVal2; // 2000 digits
	liTestVal1 *= liTestVal1; // 4000 digits
	liTestVal1 *= liTestVal1; // 8000 digits
	liTestVal1 *= liTestVal2; // 10000 digits
	liTestVal2 = liTestVal1;
...
//			writeString.Format(L"CUTOFF,%d,THREADS,%d,TIME,%d\n", i, j, duration);
			writeString.Format(L"SIZE,%d,THREADS,%d,TIME,%d\n", liTestVal1.getSize(), j, duration);
