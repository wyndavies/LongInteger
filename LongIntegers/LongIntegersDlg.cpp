...
void CLongIntegersDlg::OnClickedIdarrow()
{
	...
	// Time to test it out
	LongInteger::KARATSUBACUTOFF = 30;
	...
	liTestVal1 *= liTestVal1;
	liTestVal2 *= liTestVal2;
	LongInteger liTestResult = liTestVal1 * liTestVal2;
	CString strTestResult = liTestResult.toDecimal();
	int istrSize = strTestResult.GetLength();
	return;
